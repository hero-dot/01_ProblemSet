...
quantiles <- function(vehiclesData)
{
...
createFormattedAdWithComparisons <- function(vehicleData)
  ...
  if (vehicleData[,1] >= quantiles(vehiclesData)[2]) tempMpg <- "Top10%" else tempMpg <- ""
  if (vehicleData[,4] >= quantiles(vehiclesData)[1]) tempHp <- "Top10%" else tempHp <- ""
  if (vehicleData[,7] <= quantiles(vehiclesData)[3]) tempQsec <- "Top10%" else tempQsec <- ""
